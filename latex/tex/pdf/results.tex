...
\cref{fig:pdf-histos}. The distributions are largely compatible with
each other although there discrepancies arise in regions with low
event count (statistics), which the the ratio plot exaggerates. The
sherpa runcard utilized here and the analysis used to produce the
histograms can be found in \cref{sec:ppruncard,sec:ppanalysis}. When
comparing \cref{fig:pdf-eta,fig:histeta} it becomes apparent, that the
PDF has substantial influence on the resulting distribution.
