...
#+RESULTS:
: IntegrationResult(result=0.05380076774689264, sigma=0.0009156046547219115, N=2383)
...
#+RESULTS:
: IntegrationResult(result=0.053381188045107726, sigma=0.0009626020740618223, N=137)
...
#+RESULTS:
: VegasIntegrationResult(result=0.05353797358506919, sigma=0.00013283395611126086, N=2805, increment_borders=array([0.16380276, 0.2374099 , 0.34568281, 0.5094011 , 0.76958263,
:        1.23235422, 1.91714873, 2.37499285, 2.63412137, 2.79640826,
:        2.90423418, 2.9777899 ]), vegas_iterations=9500)
...
#+RESULTS:
: VegasIntegrationResult(result=0.053785157752621354, sigma=1.887850854134335e-05, N=2805, increment_borders=array([0.16380276, 0.23716945, 0.34540806, 0.50930035, 0.76937514,
:        1.22945366, 1.91539567, 2.37452676, 2.63392477, 2.79705411,
:        2.9048259 , 2.9777899 ]), vegas_iterations=2563)
...
#+RESULTS:
[[file:./.ob-jupyter/c858aab0b5a9a63e9de1e3666cf53a4c59da95dc.png]]
...
#+begin_src jupyter-python :exports both :results raw drawer
  ...
  reload(yplt)
  fig, _, hist_cosθ = draw_histo_auto(
      cosθ_sample,
      r"$\cos\theta$",
      title=r"own implementation",
      rethist=True,
      range=interval_cosθ,
  )
...
#+RESULTS:
[[file:./.ob-jupyter/49232645b7698576da98f5f1f2f0261107395fe9.png]]
...
#+begin_src jupyter-python :exports both :results raw drawer
  ...
  fig, ax, hist_obs_η = draw_histo_auto(
      η_sample, r"$\eta$", title="own implementation", range=interval_η, rethist=True
  )
  hist_obs_η.normalize(1)
  save_fig(fig, "histo_eta", "xs_sampling", size=[3, 3])
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/e7445bb91107a4571a4fc36cd56be9bc4ae9c72b.png]]
...
#+begin_example
  ...
  <ipython-input-202-d07958ed2945> in <module>
        7     rethist=True,
        8 )
  ----> 9 save_fig(fig, "histo_pt", "xs_sampling", size=[3, 3])

  ~/Documents/Projects/UNI/Bachelor/prog/python/qqgg/utility.py in save_fig(fig, title, folder, size)
      189
      190     fig.savefig(f"./figs/{folder}/{title}.pdf")
  --> 191     fig.savefig(f"./figs/{folder}/{title}.pgf")
      192
      193
  ...
  /usr/lib/python3.8/site-packages/matplotlib/text.py in draw(self, renderer)
      683
...
#+end_example
[[file:./.ob-jupyter/c2044eb9ceab3f0f7409038dac878edd166d0695.png]]
:END:
...
#+begin_src jupyter-python :exports both :results raw drawer
  η_hist = samples_to_yoda(η_sample, 50, title="sampled from $d\sigma / d\eta$")
  reload(yplt)
  η_hist.normalize(1)
  η_hist.setAnnotation('ratioref', True)

  yoda_plot_wrapper([hist_obs_η, η_hist], errorbars=True)
...
#+RESULTS:
:RESULTS:
| <Figure | size | 576x432 | with | 2 | Axes> | (<matplotlib.axes._subplots.AxesSubplot at 0x7f923b1ea070> <matplotlib.axes._subplots.AxesSubplot at 0x7f923b18bdc0>) |
[[file:./.ob-jupyter/6dce4295f367ad8f79bff356780e25c8093a4a1a.png]]
:END:
...
#+RESULTS:
: array([-0.9866143 , -0.96980073, -0.93072072, -0.83891589, -0.6017223 ,
:         0.00829172,  0.6071626 ,  0.83981042,  0.93107467,  0.96951636,
:         0.9866143 ])
...
#+RESULTS:
:RESULTS:
: <matplotlib.legend.Legend at 0x7f924d64e730>
[[file:./.ob-jupyter/73205cd9684fbc1ac074507ee8b55aacdfbf43f3.png]]
:END:
...
#+RESULTS:
[[file:./.ob-jupyter/cd667dd694e4f5e1dc0a7d646d140160e3be8467.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/16af6d02607855540cb4897009774f0c5f537a8b.png]]
...
#+begin_example
  ...
  <ipython-input-148-ff4d6a24ae42> in <module>
  ----> 1 fig, _ = draw_histo_auto(cosθ_sample_strat, r'$\cos\theta$')
        2 save_fig(fig, 'histo_cos_theta_strat', 'xs', size=(4,3))

  <ipython-input-132-507ea080dcd6> in draw_histo_auto(points, xlabel, bins, range, rethist, **kwargs)
       70
       71
  ---> 72     return yoda_nplot_wrapper(hist, xlabel=xlabel, **kwargs)

  <ipython-input-132-507ea080dcd6> in yoda_nplot_wrapper(*args, **kwargs)
       42 @functools.wraps(yplt.nplot)
       43 def yoda_nplot_wrapper(*args, **kwargs):
  ---> 44     fig, axs = yplt.nplot(*args, **kwargs)
       45     if isinstance(axs, iterable):
       46         axs = [set_up_axis(ax) for ax in axs]

  /usr/lib/python3.8/site-packages/yoda/plotting.py in nplot(hs, outfiles, ratio, show, nproc, **plotkeys)
      446     else:
      447         ## Run this way in the 1 proc case for easier debugging
  --> 448         rtn = [_plot1arg(args) for args in argslist]
      449
      450     if show:

  /usr/lib/python3.8/site-packages/yoda/plotting.py in <listcomp>(.0)
      446     else:
      447         ## Run this way in the 1 proc case for easier debugging
  --> 448         rtn = [_plot1arg(args) for args in argslist]
      449
  ...
  /usr/lib/python3.8/site-packages/yoda/plotting.py in _plot1arg(args)
      397 def _plot1arg(args):
      398     "Helper function for mplot, until Py >= 3.3 multiprocessing.pool.starmap() is available"
  --> 399     print(args)
      400     return plot(*args)
      401

  include/generated/Bin1D_Dbn1D.pyx in yoda.core.Bin1D_Dbn1D.__repr__()
...
: <ipython-input-151-c0fdcee231b0> in <module>
:       1 yoda_file = yoda.read("../../runcards/qqgg/analysis/Analysis.yoda")
:       2 sherpa_histos = {
: ----> 3     "pT": dict(reference=hist_obs_pt, label="$p_T$ [GeV]"),
:       4     "eta": dict(reference=hist_obs_η, label=r"$\eta$"),
:       5     "cos_theta": dict(reference=hist_cosθ, label=r"$\cos\theta$"),
:
: NameError: name 'hist_obs_pt' is not defined
:END:

#+begin_src jupyter-python :exports both :results raw drawer
  %aimport yoda.plotting
  hist = yoda_file["/MC_DIPHOTON_SIMPLE/pT"]
  hist.setAnnotation('ratioref', True)
  hist.setAnnotation('Title', "LOL")
  #hist.annotationsDict()
  fig, (ax_hist, ax_ratio) =yoda.plotting.plot([yoda_file["/MC_DIPHOTON_SIMPLE/pT"], yoda_file["/MC_DIPHOTON_SIMPLE/eta"]], ratio=True)
  set_up_axis(ax_hist)
  set_up_axis(ax_ratio)
  ax_hist.legend()
#+end_src

#+RESULTS:
:RESULTS:
: <matplotlib.legend.Legend at 0x7f924b270910>
[[file:./.ob-jupyter/bd6ab45edebf2e8126960c45a8f9897440779265.png]]
:END:
